...
```
/
├── proxy.ts                        # проверка JWT-cookie перед /admin/dashboard/** + locale-negotiation (см. §3.10)
...
**Почему страница входа вынесена в `/staff-entry`, а не лежит в `/admin/page.tsx`:** в App Router URL страницы определяется расположением папки, а не комментарием в коде. ТЗ §7.7 прямо требует, чтобы адрес входа **не был** `/admin` — если файл логина физически лежит в `/app/admin/page.tsx`, его реальный URL всё равно `/admin`, что бы ни было написано в комментарии рядом. Поэтому логин — отдельный маршрут верхнего уровня `/staff-entry`, а `/admin/dashboard/**` остаётся защищённой зоной с собственным layout и сайдбаром. Верхнего `admin/layout.tsx` без вложенного `dashboard` больше нет — он был не нужен: единственное, что он давал (отсутствие публичной навигации), уже даёт `dashboard/layout.tsx`.
...
- `proxy.ts` — проверяет httpOnly JWT-cookie только для `/admin/dashboard/**`. Страница логина (`/staff-entry`) — отдельный маршрут верхнего уровня, вне этой проверки (см. раздел 2 и пояснение под деревом файлов).
...
- `lib/rate-limit.ts` — счётчик запросов по IP (заголовок `x-forwarded-for`) с окном в несколько минут. Основной вариант — таблица в том же Postgres, по тому же принципу, что и блокировка логина: не требует внешних сервисов и работает одинаково что на Vercel, что на VPS, не нарушая принцип портируемости из раздела 6. Правило Vercel Firewall можно использовать как дополнительный, чисто платформенный уровень защиты, пока проект остаётся на Vercel, но не как единственную реализацию — иначе логика лимита привязывается к конкретному хостингу.
...
или `/de`, например по `Accept-Language`) и проверка JWT-cookie для `/admin/dashboard/**` — разные задачи,
но Next.js разрешает только один `proxy.ts` в корне. Обе логики живут в одном файле, разделённые по пути:
locale-часть применяется только вне `/admin`/`/staff-entry`, JWT-проверка — только внутри
`/admin/dashboard/**`, как и раньше (раздел 3.4). Не разносить по разным файлам — их не будет два.
...
- Индексы: `slug` (unique) на `Category`/`Product`, `productId` на `ProductVariant`, `orderId`/`productId` на `OrderItem` — под будущий рост каталога и истории заказов. Отдельно — `(ip, createdAt)` на таблице rate-limit (раздел 3.8): без него проверка лимита на каждый заказ превращается в полный скан таблицы.
...
- **Unit** (`/tests/unit`) — чистые функции в `lib/services`: расчёт цены товара (минимум среди активных вариантов), пересчёт суммы заказа, проверка блокировки логина. Не требуют БД.
- **Integration** (`/tests/integration`) — actions с тестовой БД (отдельная Neon-ветка или testcontainers): happy path `createOrder`, `adminLogin`, `resetPassword`; инвалидация сессии по `sessionVersion` после `resetPassword`; срабатывание rate limit при повторных `createOrder`.
- Jest — единый раннер для обоих уровней, как уже используется в текущей практике.
...
| §7.7 Маршрут входа не `/admin` | `/app/staff-entry/page.tsx` (вынесен из `/admin`, см. раздел 2) |
| §8 Инвалидация сессии после сброса пароля | `Admin.sessionVersion`, `lib/auth.ts` → `verifySession` (раздел 3.4) |
| §8 Авторизация | `proxy.ts` (только `/admin/dashboard/**`) + `lib/auth.ts` + `auth.service.ts` |
...
| §10 Редактирование товаров | `/app/admin/dashboard/products/*`, `products.service.ts`, `lib/storage` |
