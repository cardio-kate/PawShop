...
**Сортировка каталога без фильтра — `categoryId ASC, createdAt DESC`, не только по свежести.**
`getProducts(filters)` в `products.queries.ts` сортирует сначала по `Product.categoryId` (по возрастанию),
внутри категории — по `createdAt` (по убыванию, новое сверху). Категории заводятся сид-скриптом в порядке
Dry food(1)/Wet food(2)/Treats(3)/Accessories(4) — том же, что и в ТЗ §3 — поэтому такая сортировка без
единого дополнительного поля держит аксессуары в конце витрины без фильтра. До этой правки сортировка была
чистая `createdAt DESC`: партия товаров, заведённая позже остальных (по факту — все 6 Accessories,
добавленные последними при первом наполнении каталога), оказывалась в начале выдачи перед едой — не то,
что ожидает покупатель, открывающий каталог впервые.

**Блок «New Arrivals» — гарантированно 1 слот из еды/лакомств + 1 слот из Accessories, не «первые 2 из
isNew».** `NewArrivalsSection.tsx` делает два отдельных вызова `getProducts({ isNew: true, category, limit: 1
})` — один с категориями Dry food/Wet food/Treats, другой с Accessories — вместо одного
`getProducts({ isNew: true, limit: 2 })`. Из-за сортировки по категории выше один общий запрос с `limit: 2`
почти никогда не отдал бы Accessories (последняя категория), пока в остальных трёх суммарно ≥ 2 новинки.
`total` для «View All» — сумма `total` обоих вызовов (категории не пересекаются, сумма двух непересекающихся
подсчётов равна общему числу `isNew` без третьего запроса). Если в каком-то из двух слотов сейчас нет ни
одной новинки — тот слот пустует, блок показывает меньше двух карточек (то же уже допустимое состояние,
что и раньше при < 2 товарах с `isNew` вообще).
...
**`[STATUS 2026-08-31]` `output: 'standalone'` включается условно, не всегда.** При первом реальном
деплое на Vercel сборка падала: `Error: ENOENT: no such file or directory, open '.next/next-server.
js.nft.json'`. Причина — `standalone` меняет структуру вывода в `.next/` (свой `server.js`, другой
набор `.nft.json`) так, что билдер Vercel, ожидающий свою обычную трассировку файлов, не находит
нужный файл; Vercel-деплой и Docker/VPS-деплой через `standalone` — два взаимоисключающих режима
вывода одного и того же `next build`, не сочетание, которое можно включить одновременно ради
универсальности (`node_modules/next/dist/docs/01-app/03-api-reference/05-config/01-next-config-js/
output.md`). Исправлено: `output: process.env.VERCEL ? undefined : 'standalone'` —
`process.env.VERCEL` выставляет сама Vercel на своих билд-машинах (не заводится вручную в `.env.local`
никогда), так что на Vercel опция выключается сама, а при локальной Docker-сборке для VPS (где
`VERCEL` не задан) `standalone` остаётся как раньше.
...
этого документа и `docs/product-spec.md` — оставлено как есть по тексту, реальный файл см. в разделе 2).
