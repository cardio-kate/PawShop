...
**Тот же приём (по центру, заглавными буквами)** — и у одиночных заголовков верхнего уровня страниц-обёрток вне главной: `Catalog`, `Delivery`, `Privacy Policy` (`text-h1 uppercase text-center`, не `text-section-heading` — это `h1` страницы целиком, а не секция внутри скролла). Отступ от шапки до такого заголовка — те же `60px`. Не путать с заголовками уровня секции ВНУТРИ такой страницы (например, `You may also like` на странице товара, §7.3 Components) — те остаются обычным `h3`, по левому краю, без капса, потому что это подраздел страницы, а не весь её заголовок.
