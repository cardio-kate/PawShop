...
- **Logo / paw mark** (`logo-mark`, компонент `components/layout/Logo.tsx`): растровый актив в цвете `paw` (`#685393`, не `primary`), не CSS-токен — источник и детали см. Iconography.
...
- **LocaleSwitcher** (`components/layout/LocaleSwitcher.tsx`): `EN`/`DE` на `{typography.label-caps}`, разделены `/` (`neutral-300`, `aria-hidden`). Активная локаль — `primary` + `aria-current`; неактивная — `neutral-900` → `paw` на hover, как nav-link. `role="group"` + `aria-label="Language"` (голый `aria-label` на `div` без роли скринридером не озвучивается). Тач-таргет 24×24px — отрицательный margin поверх паддинга, визуальный интервал между `EN`/`DE` не меняется. Позиция: на десктопе — в правой группе Header между nav и поиском, `gap-sm` (8px), как у остальных иконок группы; на мобильном в основной ряд не выводится — дублируется отдельной строкой в бургер-меню.
...
- **Панель `primary-tint-panel`** — применяется только к `About` (не к Footer — см. выше). Скруглённая панель (`rounded.2xl`/40px, паддинг тоже 40px — паддинг блока должен быть не меньше его радиуса, иначе скругление визуально «съедает» контент по углам) заливает весь блок целиком, не тонкой полосой по краю. Панель — не во всю ширину фрейма, с отступом от края (белая страница видна вокруг).
  ...
  - **Страница `/catalog`** этот паттерн тоже не использует — в отличие от прежней версии (каталог блоком на главной, залитым `primary-tint`), отдельная страница каталога лежит прямо на `surface` (белый), без тинт-панели: это уже не один из нескольких блоков лендинга вперемешку с промо-контентом, а самостоятельная функциональная страница — тот же принцип, по которому админ-таблицы не наследуют «воздух» витрины (см. Layout). Фильтры + сетка товаров + пагинация — прямо на белом фоне.

## Do's and Don'ts

- Do использовать `primary` (Indigo Paw) только для действий, связанных с покупкой/заявкой (`Add to Cart`, `Checkout`, `Place Order`, выбранный вариант фасовки, активная страница пагинации) — так цвет сохраняет однозначный сигнал «действие с заказом».
...
- Do использовать `paw` (`#685393`, фактический цвет ассета лапки-лого) как единый hover/active-цвет для текстовых интерактивных элементов вне кнопок — nav-link (десктоп и бургер-меню), ссылки Footer, LocaleSwitcher, иконки Header (поиск/корзина/закрытие), кнопка закрытия `Panel`. Кнопки (`button-primary`/`button-secondary`) в это правило не входят — у них остаётся `primary-hover`/`primary-active` (см. Do выше).
