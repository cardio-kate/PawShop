...
- **Nav link** (Header, поверх §7.1 ТЗ — см. Layout): текст `neutral-700`, `label-md` (14px/600) — не `body-sm` (13px/400): пункты меню такой же по роли UI-лейбл, как текст кнопок/бейджей, а не проза, и на весе `body-sm` визуально не отличались от обычного текста страницы ниже. Без подложки; hover/active — `primary`, без подчёркивания. 4 пункта (`About`, `New Arrivals`, `Catalog`, `Contact`) — при таком небольшом каталоге (24 SKU) больше пунктов создавали бы навигацию ради навигации.
