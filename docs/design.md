...
Все три файла — **независимые экспорты одного исходника**, не производные друг от друга. Раз они не связаны программно, при следующей правке кропа/паддинга лапки нужно обновлять все три вручную — иначе они разойдутся визуально.

Важное следствие растрового формата: цвет **зашит в пиксели** — в отличие от SVG на `currentColor`, лапку нельзя перекрасить через CSS/токен. Если `colors.paw` когда-либо изменится, картинку и оба сгенерированных иконки-файла нужно пересобрать из нового источника заново, а не просто поменять значение токена в `tailwind.config.ts` — и наоборот, значение `colors.paw` в токенах должно обновляться вслед за пересборкой ассета, а не жить своей жизнью (см. выше — уже один раз разошлись). По той же причине лапку нельзя использовать там, где нужен другой цвет/оттенок относительно фона (например, белым на тёмной подложке) — для таких случаев нужна отдельная версия ассета, а не CSS-перекраска.
